...
class SepFormer(nn.Module):
    ...
    def forward(self, input, embed):
        output, _ = self.extract_latent(input, embed)

        return output

    def extract_latent(self, input, embed):
        """
        Args:
...
class SepFormerBlock(nn.Module):
    def __init__(
        ...
        num_heads_intra=8, num_heads_inter=8,
        d_intra=256, d_inter=256, d_ff_intra=1024, d_ff_inter=1024, d_emb=1024,
        norm=True, dropout=1e-1, nonlinear='relu',
        causal=False,
        eps=EPS
    ):
        super().__init__()
        self.intra_e_proj = nn.Linear(d_emb, d_intra)
        self.inter_e_proj = nn.Linear(d_emb, d_inter)
    ...
    def forward(self, input, embed):
        ...
        Returns:
            output (batch_size, num_features, S, chunk_size)
        """
        intra_embed = self.intra_e_proj(embed)
        inter_embed = self.inter_e_proj(embed)
        x = self.intra_transformer(input, intra_embed)
        output = self.inter_transformer(x, inter_embed)
...
class IntraTransformer(nn.Module):
    ...
    def forward(self, input, embed):
        ...
        embed = embed[None, :, None:].repeat(1, 1, S, 1) # (batch_size, num_features) -> (1, batch_size, S, num_features)
        embed = embed.reshape(1, batch_size * S, num_features) # (1, batch_size * S, num_features)
        ...
        x = torch.cat([x, embed], dim=0) # (chunk_size + 1, batch_size * S, num_features)
        embedding = self.positional_encoding(x)
        x = x + embedding
        
        x = self.transformer(x) # (chunk_size + 1, batch_size * S, num_features)
        x = x[:-1, ...] # (chunk_size + 1, batch_size * S, num_features) -> (chunk_size, batch_size * S, num_features)
        x = x.view(chunk_size, batch_size, S, num_features) # (chunk_size, batch_size * S, num_features) -> (chunk_size, batch_size, S, num_features)
...
class InterTransformer(nn.Module):
    ...
    def forward(self, input, embed):
        ...
        num_features = self.num_features
        batch_size, _, S, chunk_size = input.size()
        embed = embed[None, :, None:].repeat(1, 1, chunk_size, 1) # (batch_size, num_features) -> (1, batch_size, chunk_size, num_features)
        embed = embed.reshape(1, batch_size * chunk_size, num_features) # (1, batch_size * chunk_size, num_features)
        ...
        x = torch.cat([x, embed], dim=0) # (S, batch_size * chunk_size, num_features) -> (S + 1, batch_size * chunk_size, num_features)
        embedding = self.positional_encoding(x)
        x = x + embedding
        x = self.transformer(x) # (S + 1, batch_size*chunk_size, num_features)
        x = x[:-1, ...] # (S + 1, batch_size * chunk_size, num_features) -> (S, batch_size * chunk_size, num_features)
        x = x.view(S, batch_size, chunk_size, num_features) # (S, batch_size * chunk_size, num_features) -> (S, batch_size, chunk_size, num_features)
